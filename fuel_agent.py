...
def fetch_prices(url):
    ...
    for row in rows:
        ...
        if len(cols) >= 2:
            ...
            price_text = cols[1].get_text(strip=True).replace(',', '.').replace('грн', '').strip()
    ...
    with open(HISTORY_FILE, 'a', newline='', encoding='utf-8') as f:
        ...
        for network, prices in all_prices.items():
            for fuel, price in prices.items():
                writer.writerow({'date': date_str, 'network': network, 'fuel': fuel, 'price': price})
...
def main():
    ...
    all_prices = {}
    for network, url in SOURCES.items():
